...
class Trainer:
    ...
    def build_model(self, pretrain_weights=None, device=None):
        """Build DCENet Model

        Args:
            pretrain_weights: Path to pre-trained weights
            device: torch.device object or string ('cpu' or 'cuda')
        """
        if device is None:
    ...
    def infer_cpu(self, image_pil, image_resize_factor=None):
        """Infer on CPU with PIL image input"""
        with torch.no_grad():
            ...
            lowlight = (np.asarray(image_pil) / 255.0)
        ...
        return image_pil, enhanced.numpy()
